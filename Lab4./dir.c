...
ret = osfs_alloc_continuous_blocks(sb_info, 3, &osfs_inode->i_start_block);
osfs_inode->i_blocks_count = 3;
osfs_inode->i_blocks = 3;
...
{   
    ...
    parent_inode->__i_mtime = now;
    parent_inode->__i_ctime = now;
...
// 在 dir.c 中加入此函式
static int osfs_symlink(struct mnt_idmap *idmap, struct inode *dir,
                        struct dentry *dentry, const char *target)
{
    struct inode *inode;
    struct osfs_inode *os_inode;
    struct osfs_sb_info *sb_info = dir->i_sb->s_fs_info;
    int ret;
    size_t len = strlen(target) + 1; // 包含 null 終止符

    if (len > BLOCK_SIZE)
        return -ENAMETOOLONG;

    // 1. 建立一個類型為 S_IFLNK (Symbolic Link) 的新 Inode
    inode = osfs_new_inode(dir, S_IFLNK | 0777);
    if (IS_ERR(inode))
        return PTR_ERR(inode);

    os_inode = inode->i_private;

    // 2. 將目標路徑 (target) 寫入該 Inode 分配到的資料區塊
    char *data = (char *)(sb_info->data_blocks + os_inode->i_block * BLOCK_SIZE);
    memcpy(data, target, len);
    
    // 設定檔案大小為路徑長度
    inode->i_size = len - 1;
    os_inode->i_size = len - 1;
...
const struct inode_operations osfs_dir_inode_operations = {
    ...
    .symlink = osfs_symlink, // 註冊此進入點以支援 ln -s
};
