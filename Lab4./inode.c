...
struct inode *osfs_iget(struct super_block *sb, unsigned long ino)
{
    ...
    // --- 新增以下 Bonus 部分 ---
        ...
        inode->i_op = &osfs_symlink_inode_operations; // 連結檔不需要 fop
    }
...
// 在 inode.c 中新增或修改分配邏輯
int osfs_alloc_continuous_blocks(struct osfs_sb_info *sb_info, uint32_t count, uint32_t *start_block) {
    uint32_t i, j;
    for (i = 0; i <= sb_info->block_count - count; i++) {
        bool found = true;
        for (j = 0; j < count; j++) {
            if (test_bit(i + j, sb_info->block_bitmap)) {
                found = false;
                break;
            }
        }
        if (found) {
            for (j = 0; j < count; j++) {
                set_bit(i + j, sb_info->block_bitmap);
            }
